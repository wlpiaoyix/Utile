...
- (void)layoutSubviews {
    ...
    if (label.text && label.text.length) {
        labelSize = [PYUtile getBoundSizeWithTxt:label.text font:label.font size:CGSizeMake(999, 20)];
        if (labelSize.width<bounds.size.width/2) {
            labelSize = [PYUtile getBoundSizeWithTxt:label.text font:label.font size:CGSizeMake(bounds.size.width, 999)];
        }
    }
    ...
    CGSize detailsLabelSize = [PYUtile getBoundSizeWithTxt:detailsLabel.text font:detailsLabel.font size:maxSize];
